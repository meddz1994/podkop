...
install_awg_packages() {
    ...
    if opkg list-installed | grep -q amneziawg-tools; then
        ...
        AMNEZIAWG_TOOLS_FILENAME="amneziawg-tools_v24.10.0_aarch64_generic_armsr_armv8.ipk"
        DOWNLOAD_URL="https://github.com/Slava-Shchipunov/awg-openwrt/releases/download/v24.10.0/${AMNEZIAWG_TOOLS_FILENAME}"
        wget -O "$AWG_DIR/$AMNEZIAWG_TOOLS_FILENAME" "$DOWNLOAD_URL"

        if [ $? -eq 0 ]; then
            echo "amneziawg-tools file downloaded successfully"
        else
            echo "Error downloading amneziawg-tools. Please, install it manually and run the script again"
            exit 1
        fi
    ...
    if opkg list-installed | grep -q luci-app-amneziawg; then
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "Error downloading luci-app-amneziawg. Please, install it manually and run the script again"
            exit 1
        fi
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "Error installing luci-app-amneziawg. Please, install it manually and run the script again"
            exit 1
        fi
    fi
